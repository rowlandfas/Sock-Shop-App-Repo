      ...
      steps {
          script {
              ...
              sh '''
                  ...
                  SSM_PID=$!
                  echo $SSM_PID > /tmp/ssm-session.pid
              ...
              sshagent(['bastion-key', 'ansible-key']) {
                  sh '''
                      ...
                      ssh \
                          -o StrictHostKeyChecking=no \
                          -o ConnectTimeout=10 \
                          -o ProxyCommand="ssh -W %h:%p -o StrictHostKeyChecking=no -p 9999 ubuntu@localhost" \
                          ubuntu@${ANSIBLE_IP} \
              ...
              sh '''
                  ...
                  pkill -f "aws ssm start-session" 2>/dev/null || true
              '''
          }
      }
  }
